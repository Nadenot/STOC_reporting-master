...
bg <- sort(setdiff(unique(subset(d,NEW.ID_PROG == site)$BG),bagueur))
nbb <- length(bagueur)
...
subtitle_r <-  paste0("\\vspace{5cm} \\large ",bagueur_r," \\vspace{1cm}")#\\normalsize ",bagueur_r,"\\vspace{1cm}")
...
On remercie toutes les personnes qui ont participer au maintient et à l'amélioration du scritp de production de ce rapport, 
 Benboudjema Sophie (Mme) <sophie.benboudjema@polytechnique.edu>
...
d_session <- unique(subset(d,NEW.ID_PROG == site,select=c("YEAR","SESSION","DATE","FS.DEDUIT","FS.METHODE")))
